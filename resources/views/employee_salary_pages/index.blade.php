...
<div class="container mb-5">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div style="color:cornsilk" class="card-header d-flex justify-content-center bg-dark">{{ __('Search Employee Balance') }}</div>

                <div class="card-body">
                        {{-- {{route('employee_salary.search')}} --}}


                        <form method="POST" action="{{route('employee_salary.search')}}">
                            @csrf
  ...
  <?php $c =1 ?>
    ...
    @foreach($all_users as $user)
      <tr>
        ...
        <td>{{$user->title}}</td>
        <td><a href="{{url('employees_salary/' . $user->id) }}" class="btn btn-primary float-left mr-3"> Show Salary Details</a> </td>
      </tr>
      <?php $c++ ?>
    @endforeach  
    </tbody>
